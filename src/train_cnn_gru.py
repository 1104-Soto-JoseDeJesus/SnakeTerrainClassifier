...
RESULTS_FOLDER = "results/cnn_gru"
...
class SnakeTerrainCNNGRU(nn.Module):
    """
    CNN-GRU model for time-series terrain classification.
    ...
    CNN part:
    learns local vibration / spike / oscillation patterns.

    GRU part:
    learns how those patterns evolve over time.
    """
...
def plot_training_curves(train_losses, test_accuracies):
    ...
    plt.title("CNN-GRU Test Accuracy by Epoch")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.grid(True)
    plt.tight_layout()
...
def plot_confusion_matrix(cm, class_names):
    output_path = os.path.join(RESULTS_FOLDER, "cnn_gru_confusion_matrix.png")
...
def main():
    ...
    model_path = os.path.join(RESULTS_FOLDER, "snake_terrain_cnn_gru_model.pth")
    ...
    results_text_path = os.path.join(RESULTS_FOLDER, "cnn_gru_results.txt")

    with open(results_text_path, "w") as f:
        f.write("CNN-GRU Deep Learning Results\n")
        f.write("=============================\n\n")
