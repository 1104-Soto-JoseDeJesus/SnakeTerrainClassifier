...
RESULTS_FOLDER = "results/multiseed_ensemble"
...
def make_extra_trees(params, seed):
...
def average_probabilities(models, X_data, num_classes):
    """
    Averages predict_proba outputs from models.

    Handles models that may only know a subset of classes,
    such as bumpy-vs-gravel specialists.
    """
    ...
    average_probabilities = total_probabilities / len(models)
...
def main():
    ...
    print()
    print("Using Extra Trees params:")
    print(params)
    ...
    for seed in SEEDS:
        print()
        print(f"Training seed {seed} models...")

        general_model = make_extra_trees(params, seed)
        general_model.fit(X_train, y_train)
        general_models.append(general_model)

        rough_model = make_extra_trees(params, seed + 1000)
        rough_model.fit(X_train[rough_train_mask], y_train[rough_train_mask])
        rough_models.append(rough_model)

        regular_model = make_extra_trees(params, seed + 2000)
        regular_model.fit(X_train[regular_train_mask], y_train[regular_train_mask])
        regular_models.append(regular_model)

    print()
    print("Averaging general model probabilities...")
    ...
    print()
    print("Applying multiseed specialist correction...")
    ...
    corrected_accuracy = evaluate_predictions(
        "multiseed_specialist_corrected_extra_trees",
        y_test,
        corrected_predictions,
        class_names
    )

    summary_path = os.path.join(RESULTS_FOLDER, "multiseed_ensemble_summary.txt")

    with open(summary_path, "w") as f:
        f.write("Multiseed Ensemble Summary\n")
        f.write("==========================\n\n")
        f.write(f"Seeds: {SEEDS}\n\n")
        f.write(f"Multiseed general Extra Trees: {general_accuracy:.4f}\n")
        f.write(f"Multiseed specialist-corrected Extra Trees: {corrected_accuracy:.4f}\n\n")
        f.write("Previous best:\n")
        f.write("Specialist-corrected Extra Trees: 0.8575\n")

    print()
    print("Multiseed ensemble summary:")
    print(f"Multiseed general Extra Trees: {general_accuracy:.4f}")
    print(f"Multiseed specialist-corrected Extra Trees: {corrected_accuracy:.4f}")
    print()
